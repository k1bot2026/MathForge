import { describe, expect, test } from "vitest";
import type { FunctionPayload, MathValue } from "~/math/types";

const ctx = { signal: new AbortController().signal };

function makeFn(expression: string): MathValue {
  const payload: FunctionPayload = { expression, variables: ["x"] };
  return {
    type: {
      kind: "Function",
      arity: 1,
      domain: { kind: "Scalar", field: "real", precision: "approximate" },
      codomain: { kind: "Scalar", field: "real", precision: "approximate" },
    },
    payload: payload as unknown as number,
    provenance: { blockId: "calc.function", inputs: [], computedAt: 0, engine: "sympy" },
  };
}

describe("viz.taylor compute", () => {
  test("returns fn passthrough when fn is provided", async () => {
    const { VizTaylorBlock } = await import("./definition");
    const fn = makeFn("cos(x)");
    expect(VizTaylorBlock.compute({ fn }, {}, ctx)).toBe(fn);
  });

  test("throws when fn input is missing", async () => {
    const { VizTaylorBlock } = await import("./definition");
    expect(() => VizTaylorBlock.compute({}, {}, ctx)).toThrow(
      "viz.taylor requires f(x) on the fn port",
    );
  });
});

describe("viz.taylor definition explain", () => {
  test("effect returns connect prompt when fn is missing", async () => {
    const { VizTaylorBlock } = await import("./definition");
    const effect = VizTaylorBlock.explain.effect;
    if (effect === undefined) throw new Error("effect undefined");
    expect(effect({}, undefined as never)).toMatch(/Connect f\(x\)/);
  });

  test("effect returns overlay prompt when fn is connected but taylor is missing", async () => {
    const { VizTaylorBlock } = await import("./definition");
    const effect = VizTaylorBlock.explain.effect;
    if (effect === undefined) throw new Error("effect undefined");
    const msg = effect({ fn: makeFn("cos(x)") }, undefined as never);
    expect(msg).toMatch(/Taylor polynomial/);
  });

  test("effect returns overlay confirmation when both fn and taylor connected", async () => {
    const { VizTaylorBlock } = await import("./definition");
    const effect = VizTaylorBlock.explain.effect;
    if (effect === undefined) throw new Error("effect undefined");
    const msg = effect({ fn: makeFn("cos(x)"), taylor: makeFn("1 - x**2/2") }, undefined as never);
    expect(msg).toMatch(/Taylor approximation overlay/);
  });
});
