...
export function computeAdd(inputs: ResolvedInputs): MathValue {
  const A = inputs.A;
  const B = inputs.B;
  if (A === undefined || B === undefined) {
    throw new AddError("add requires both A and B inputs");
  }
  ...
  if (am !== bm || an !== bn) {
    throw new AddError(`Shape mismatch: A is ${am}×${an}, B is ${bm}×${bn}`);
  }
